...
def countFingers(image, results ,computer_choice, status,draw=True, display=True):
    ...
    if draw:
        ...
        if status == 0: #lose
            cv2.putText(output_image, "You Lost!" , (50, 90),cv2.FONT_HERSHEY_COMPLEX, 3, (0, 0, 255), 2)
        elif status == 1: #win
            cv2.putText(output_image, "You Won!" , (50, 90),cv2.FONT_HERSHEY_COMPLEX, 3, (0,255,0), 2)
        elif status == 2: #draw
            cv2.putText(output_image, "Draw!" , (50, 90),cv2.FONT_HERSHEY_COMPLEX, 3, (0,0,0), 2)
...
while camera_video.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        frame, fingers_statuses, count = countFingers(frame, results,computer_choice, status , display=False)
