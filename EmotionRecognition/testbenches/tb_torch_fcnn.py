...
from torch_model_classes import FCNNModel, ResNet, to_device, get_default_device
from math import pi
...
fcnn_ck.load_state_dict(torch.load(model_path + 'FCNN_norm_128_ck.pt')['state_dict'])
fcnn_ck = fcnn_ck.cuda()
fcnn_ck.eval()

fcnn_fer = FCNNModel(68*4, 128, 7) # input, hidden, output
fcnn_fer.load_state_dict(torch.load(model_path + 'FCNN_norm_128_fer.pt')['state_dict'])
fcnn_fer = fcnn_fer.cuda()
fcnn_fer.eval()
...
def start():
    ...
    while True:
        ...
        # prediction
        with torch.no_grad():
            pred_tensor_ck = fcnn_ck(torch.Tensor(Vector).cuda()).argmax()
            pred_ck = pred_tensor_ck.cpu().numpy().item()
            pred_tensor_fer = fcnn_fer(torch.Tensor(Vector).cuda()).argmax()
